...
def compare_offspring_to_parent(filename, offspring_gen, a):
            ...
            if not org.lower().startswith("filename"):
                ...
                #get the data for the two partents of the org
                    for org_parent in file_rows:
                        ...
                        if offspring_gen > 1 and (p1 and p2):
                            mean_parent_co_points = (float(parent2_xovers) +float((parent1_xovers)))/2.0
                            #print (parent1_xovers,parent2_xovers, mean_parent_co_points)
                            ...
                            org_row = [child_name,child_fitness,child_xovers,
                                        parent1_name, parent1_fitness, parent1_xovers,
                                        parent2_name, parent2_fitness, parent2_xovers,
                                        mean_parent_co_points,selection_differential_co,
                                        mean_parent_fitness,selection_differential_fitness]
                        else:
                            pass
                    csv_rows.append(org_row)
        results_file = open(a, 'a')
...
a = 'no_devo_parent_offspring_comparison.csv'
results_file = open(a, 'w')
wr = csv.writer(results_file, dialect='excel')
wr.writerow(['child','child_fitness','child_xover'
             'parent1', 'parent_1_fitness','parent1_co_points',
             'parent2','parent2_fitness','parent2_co_points' ,
             'mean_parent_co_points','selection_differential_co_points',
             'mean_parents_fitness','selection_differential_fitness'])
results_file.close()
for i in range(2,10):
    compare_offspring_to_parent(f, i, a)
