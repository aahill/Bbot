...
#+BABEL: :session *python* :cache yes of having development being sensitive to evolution. 
...
** COMMENT Generting initial population 
#+BEGIN_SRC python :results output
#+END_SRC

#+RESULTS:

#+BEGIN_SRC python :results output :tangle genOrgs.py
  import Organism
  def gen_orgs():
      viable = False
      path_devo = '/home/jake/org/Thesis_Stuff/Robot_Data/Development/Gen1'
      path_no_devo = '/home/jake/org/Thesis_Stuff/Robot_Data/Non_Development/Gen1'
      while(not viable):
          org = Organism.Organism(1, 3, 560, 2, True, 80, 2000)
          genome =org.genome 
          non_devo_org = Organism.Organism(1,3, 560, 2, True, 80, 2000, None, None,genome, True)
          if org.is_viable() and (len(org.connections) != len(non_devo_org.connections)):
              devo_connecions = [i.group_id for i in org.connections]
              nodevo_connecions = [i.group_id for i in non_devo_org.connections]
              print "devo",devo_connecions
              print "nodevo", nodevo_connecions
              org.save_to_file(path_devo)
              non_devo_org.save_to_file(path_no_devo)
              print "Huzzah"
              viable =True
#+END_SRC

#+RESULTS:
: devo ['fl', 'i6', 'bl', 'pl', 'rr', 'br']
: nodevo ['fl', 'i6', 'bl', 'pl', 'rr', 'rr', 'br', 'i6']
: Huzzah
...
#+BEGIN_SRC python :results output
  ...
  clone_diffs = [1117,171,11, 875, 130, 33, 1133]
  clone_mean = 441.75
  non_clone_diffs = [32, 1189]
  none_clone_mean = 610.5
  print "clone stderr: %s\n nonclone stderr: %s" % (calculateStdError(clone_diffs, clone_mean), calculateStdError(non_clone_diffs, none_clone_mean))
#+END_SRC
...
#+tblname: tbl
| 1 | No Interactions      | 441.75 | 183.203379653 |
| 2 | Interactions         |    610 | 409.061272916 |


#+BEGIN_SRC gnuplot :var data=tbl :file clones_v_nonclones.png
  reset
  set title 'Clones v. nonclones in Gen1'
  set xlabel ''
  set xrange[0:3]
  set yrange [100:1200]
  set xtics rotate out
  set ylabel 'lux'
  plot '$data' using 1:3:4:xticlabels(2) with errorbars title 'Mean of differences'

#+END_SRC

#+RESULTS:
[[file:clones_v_nonclones.png]]


* Experiment notes 
** <2016-03-05 Sat>
Today we re-ran all of Gen1
- Replaced 1_1 and 1_3 with an interaction genome
- Reran 1_5 and 1_9 becaause they were the biggest outliers
- 
** <2016-03-07 Mon>
- No_devo_4_2... robot moving in a way that should not happen based on it wiring.
  - This makes me think that evolution is exploiting quirks in the hardware
of the bot.
