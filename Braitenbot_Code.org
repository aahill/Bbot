          ...
          for running in running_threads:
              ...
              if running in active_threads:
                  ...
                  try:
                      ...
                      if running in active_threads:
                          active_threads.remove(running)
                  if not error_encountered and new_connection_origin != None:
                      try:
                          ...
                          if self.alt_mode:
                              ...
                              if running in active_threads:
                                  active_threads.remove(running)
                  #if not adding the new connection origin, check if it's already taken, and if not, set it to available
                  if error_encountered and new_connection_origin != None:
                      if new_connection_origin not in self.connections:
